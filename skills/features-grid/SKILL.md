---
name: features-grid
description: "Print a colorful ASCII CARD-GRID of a project's features/changes — done vs in-progress, every card with a progress bar — DIRECTLY IN THE REPLY. Same skill as /changes-visualizer (kept under this name because it's the habitual command). A bundled Python script does all the drawing/coloring from a JSON spec; the agent gathers honest data, runs the script, and PASTES its output into the message inside a fenced code block. The whole point (and the thing that kept failing before): the boxes must appear IN THE MESSAGE THE USER READS — not buried in a tool/bash-output block, not summarized, not narrated. Use on /features-grid, 'features grid', 'feature cards', 'show features/changes as cards/boxes', 'done vs in-progress features', 'feature dashboard with progress bars', or any ask for a visual per-feature/per-change card overview."
argument-hint: "[what to card up — e.g. 'this session's changes', 'done vs todo', a project path]"
---

# /features-grid — colored feature/change cards, PRINTED IN THE REPLY

Same behavior as `/changes-visualizer`. A bundled script draws pixel-perfect ASCII cards
(two sections — DONE and IN PROGRESS — each card a rounded box with a colored progress bar)
from a JSON spec. You never hand-draw boxes. Your job: gather honest data, run the script,
and **print its output in your reply**.
...
1. **Gather the features/changes**, newest-first, split into two buckets, from whatever
...
3. **Run the renderer** (absolute path so aliases resolve):
   ```bash
   python3 ~/.claude/skills/features-grid/scripts/changes_grid.py --no-color <spec.json>
   # colored variant for a live terminal (do NOT paste this one into the message):
   python3 ~/.claude/skills/features-grid/scripts/changes_grid.py <spec.json>
   ```
...
## Gotchas (burned live 2026-07-06)
...
---
*Provenance: rebuilt via /dnl on 2026-07-06 to enforce print-in-reply. Twin of
`/changes-visualizer` (same renderer + rules). The pre-rewrite version is stashed under
