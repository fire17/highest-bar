...
- No API keys anywhere — transcription is LOCAL (mlx-whisper / openai-whisper).
"""
...
#
# Transcription is LOCAL — mlx-whisper (Apple Silicon) or openai-whisper (CPU).
# No API key, no upload: audio never leaves this machine. Used only when yt-dlp
# cannot get captions (or for local files with no subtitles). Install an engine:
#   pip3 install mlx-whisper      (preferred on Apple Silicon)
#   pip3 install openai-whisper   (CPU fallback)
# Without one, /watch still works — captionless videos come back frames-only.
...
def _have_whisper_engine() -> tuple[bool, str | None]:
    """Detect a LOCAL whisper engine (no API keys in this fork)."""
...
def _status() -> dict:
    ...
    has_whisper, backend = _have_whisper_engine()
    ...
    can_proceed = (not missing) and (has_whisper or setup_complete)
...
def cmd_check() -> int:
    ...
    if not s["has_whisper"] and not s["setup_complete"]:
        parts.append("no local whisper engine (mlx-whisper / openai-whisper)")
...
def cmd_install() -> int:
    ...
    if has_whisper:
        ...
        print(f"[setup] ready. local whisper backend: {backend}")
        if installed_deps:
            print("[setup] installed dependencies; /watch is fully set up.")
        return 0

    print("")
    print("[setup] one step left: install a LOCAL whisper engine (no API key needed).")
    print("")
    print(f"  {_whisper_install_hint()}")
    print("")
    print("  Without one, /watch still works but videos without captions come back frames-only.")
    return 3
