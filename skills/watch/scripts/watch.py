...
from local_whisper import load_api_key, transcribe_video  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--whisper",
        choices=["mlx", "openai-whisper", "lightning"],
        default=None,
        help="Force a specific LOCAL whisper engine. Default: prefer mlx-whisper "
             "(Apple Silicon), fall back to openai-whisper (CPU). "
             "'lightning' is opt-in (lightning-whisper-mlx, batched decode).",
    )
    ap.add_argument(
        "--cookies-from-browser",
        type=str,
        default=None,
        help="Read cookies live from a local browser (chrome, firefox, safari, edge, "
             "brave, …) for login-gated sources. Never copied, stored, or logged.",
    )
    ap.add_argument(
        "--cookies",
        type=str,
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        if backend and api_key:
            ...
            hint = (
                f"--whisper {args.whisper} was set but that engine is not installed"
                if args.whisper else
                "no subtitles and no local whisper engine installed"
            )
            setup_py = SCRIPT_DIR / "setup.py"
            print(
                f"[watch] {hint} — run `python3 {setup_py}` for the exact pip3 install command",
                file=sys.stderr,
            )
    ...
    if transcript_text:
        ...
        print(
            ...
            "Captions were missing and the local whisper fallback was unavailable "
            "(no engine installed, or `--no-whisper` was used). "
            f"Run `python3 {setup_py}` for the install command, then re-run._"
        )
